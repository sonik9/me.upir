...
(function ($) {
    ...
    function TimelineCV(container, configuration) {
        ...
        _this.containerDetail = $(_this.config.containerDetail);

        /*For skills only*/
        _this.containerSkills = $(_this.config.containerSkills);
        _this.containerSkills.append(_this.createSkillsMixElement(_this.config.data.skills));

        _this.buttonTypeChange = $('<div/>').addClass(_this.config.timelineButtonsClass).addClass('button-type-change')
            .append($('<i/>').addClass(_this.config.iconClass[3]));
        _this.buttonDirectionChange = $('<div/>').addClass(_this.config.timelineButtonsClass).addClass('button-direction-change')
            .append($('<i/>').addClass(_this.config.iconClass[2]));

        _this.buttonScrollContainer = $('<div/>').addClass('btn-timeline-scroll-container');
        var btnIconLeft = $('<i/>').addClass('fa fa-angle-left');
        var btnIconRight = $('<i/>').addClass('fa fa-angle-right');
        _this.buttonScrollLeft = $('<div/>').addClass('btn').addClass('button-scroll-left').append(btnIconLeft);
        _this.buttonScrollRight = $('<div/>').addClass('btn').addClass('button-scroll-right').append(btnIconRight);
        _this.buttonScrollContainer.append(_this.buttonScrollLeft).append(_this.buttonScrollRight);
        /**
    ...
    TimelineCV.prototype.reload = function () {
        ...
        $(this.containerDetail).find('.detail-block').remove();
        $(this.buttonDirectionChange).remove();
        $(this.buttonTypeChange).remove();
        $(this.buttonScrollContainer).remove();
        this.init(this.config.data);
        if ($(this.container).find('.timeline-cv').height() < $(window).height()) {
            //debugger;
    ...
    TimelineCV.prototype.init = function (data) {
        ...
        if (_this.config.typeOfView === 'vertical') {
            _this.container.append(_this.buttonTypeChange).append(_this.buttonDirectionChange);
        } else {
            _this.container.append(_this.buttonScrollContainer);
            _this.container.append(_this.buttonDirectionChange);
        }
        ...
        _this.containerDetail.append(_this.createDescDetailElement(_array));
    ...
    TimelineCV.prototype.elementEvents = function () {
        //events
        var _this = this;
        _this.buttonTypeChange.click(function (e) {
            //e.preventDefault();
        ...
        _this.buttonDirectionChange.click(function (e) {
            //e.preventDefault();
        ...
        _this.buttonScrollRight.click(function () {
            console.log('right');
            var pos = $('.timeline-cv').scrollLeft() + $(window).width() - 100;
    ...
    TimelineCV.prototype.createMainLineElement = function (array) {
        ...
        function blockCrete(item, arrayLength, year, i) {
            ...
            if (_this.config.typeOfTimeline === "expanded") {
                itemBlock.addClass('expanded-vertical');
    ...
    TimelineCV.prototype.descElementVertical = function (array) {
        ...
        function createElementEventLine(item) {
            ...
            var endDateTop = mainAreaEnd.position().top;
            //event line
            var halfBlockTop = startDateTop + _this.config.mainBlockArea;
            var halfBlockBottom = endDateTop + _this.config.mainBlockArea;
            var elementTop = halfBlockTop + (_this.config.heightMainArea / 12) * _getYearMonth(item.startDate, 'm');
            var elementBottom = halfBlockBottom + (_this.config.heightMainArea / 12) * _getYearMonth(item.endDate, 'm');
            var elementHeight = (elementBottom - elementTop);
            var margin = 15;
            ...
            function cross(array, delta) {
                if (array.length > 0) {
                    $.each(array, function (i, a) {
                        var x = a.offsetTop;// - $this.container.children().filter('.' + _this.config.ulContainerClass).offset().top;
                        var y = x + $(a).outerHeight();
                        var leftItem = $(a).css('margin-left');
                        var leftCurr = lineEvent.css('margin-left');
                        if ((elementTop >= x && elementTop < y) || (elementBottom >= x && elementBottom < y)) {
    ...
    TimelineCV.prototype.createDescDetailElement = function (array) {
        ...
        var jobArticleHeader = $('<h4/>').html('Work expirience');
        var eduArticleHeader = $('<h4/>').html('Education');
    ...
    TimelineCV.prototype.createSkillsMixElement = function (array) {
        var container = $('<div/>', {'class': 'skills-container'});

        array.forEach(function (item, i) {
            var skillBlock = $('<ul/>');
            var skillName = $('<div/>').append($('<h4/>').html(item.name));
            skillBlock.append(skillName);

            item.keywords.forEach(function (item, i) {
                var skillList = $('<li/>');
                var strSeparate = item.split(':');
                var skillSubName = $('<div/>', {'class': 'skills-subname'}).html(strSeparate[0]);
                var skills = $('<div/>', {'class': 'skills-list'});
                if (strSeparate.length > 1) {
                    // debugger;
                    var skillsArray = strSeparate[1].split(',');

                    skillsArray.forEach(function (item, i) {
                        var skill = $('<a/>',
                            {'class': 'skill-tag', 'href': 'https://www.google.fi/search?q=' + item, 'target': '_blank'}
                        ).html(item);
                        skills.append(skill);
                    });
                    skillList.append(skillSubName).append(skills);
                } else {
                    skillList.append(skillSubName);
                }

                skillBlock.append(skillList);
            });
            container.append(skillBlock);
        });
        return container;
    };
